...
export class PushNotificationRepository {
  ...
  // Per-company late/unpaid invoice counts for the daily digest — only
  // companies with at least one NON_PAYEE invoice in either bucket are
  // returned, so ReminderCronService never has to filter out zero-count
  // rows itself.
  async findReminderCounts(now: Date): Promise<ReminderCounts[]> {
    const [late, unpaid] = await Promise.all([
    ...
    for (const row of unpaid) {
      const existing = counts.get(row.companyId);
      if (existing) {
  ...
  async markReminded(companyIds: string[], now: Date): Promise<void> {
    ...
    await this.prisma.invoice.updateMany({
      where: { companyId: { in: companyIds }, status: 'NON_PAYEE' },

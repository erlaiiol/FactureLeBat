...
if [ ! -f "$BACKEND_ENV" ]; then
	...
	*'*'*) warn "CORS_ORIGIN contient '*' dans $BACKEND_ENV — probablement une erreur de config plutôt qu'un vrai wildcard voulu" ;;
	...
	audit_feature_group "Connexion Apple" "APPLE_CLIENT_ID" "$BACKEND_ENV"
	audit_feature_group "Connexion Apple (web/Android)" "APPLE_SERVICES_ID" "$BACKEND_ENV"
...
if [ ! -f "$INFRA_ENV" ]; then
	...
	*localhost*)
	...
	audit_feature_group "Connexion Apple" "APPLE_CLIENT_ID" "$INFRA_ENV"
	audit_feature_group "Connexion Apple (web/Android)" "APPLE_SERVICES_ID" "$INFRA_ENV"

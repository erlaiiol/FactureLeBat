...
if [ ! -f "$BACKEND_ENV" ]; then
	...
	*'*'*) warn "CORS_ORIGIN contient '*' dans $BACKEND_ENV — probablement une erreur de config plutôt qu'un vrai wildcard voulu" ;;
	...
	info "Connexion Apple : pas encore implémentée (bouton désactivé côté frontend uniquement) — nécessite un compte Apple Developer Program (Services ID + clé privée) avant de pouvoir définir ses propres variables ici, voir docs/roadmap.md Phase 1.5"

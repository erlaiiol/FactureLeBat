...
### Discount lines (Phase 32)

A `Discount` (`discount/`) is a catalog entry mirroring `Product`/`Service` — deliberately a shorter shape (name + `discountType` + `fixedAmountCents`/`percentageBasisPoints` only, no code/description/activityCategory), matching the quick mode's third "+" button (alongside produit/prestation). Adding one to an invoice creates an `InvoiceDiscountLine` — a soft reference to the `Discount` (same "autofill, not a lock" rule) with its own snapshotted `name`/`amountCents`.

Unlike a service line, a discount line has no visibility/redistribution axis: it always folds its `amountCents` straight into the subtotal as a reduction. `InvoiceMapper` computes `subtotalExclVatCents` as `sum(lines) + sum(visible serviceLines) - sum(discountLines)`, floored at 0 — so `subtotalExclVatCents + vatAmountCents === totalInclVatCents` always holds, the same invariant every other consumer (reports, board, PDF) already relies on. `PdfService.buildTotals` reconstructs the pre-discount figure for display (`subtotalExclVatCents + discountTotalCents`) purely for the "Sous-total HT / <remise> / Total HT" breakdown; it is never a second source of truth.

A `PERCENTAGE` discount (like a `PERCENTAGE` service) is resolved to a concrete `amountCents` client-side, against `InvoiceDraftStore.percentageBaseCents` (product lines + FIXED visible service lines, before VAT — the same base a `PERCENTAGE` service line uses) at the moment it's added to the draft. The backend never sees a rate, only the resolved `amountCents` — same "computed at build time, not typed per invoice" precedent, and the same reason a persisted invoice's discount line can't be reopened as a percentage on edit (it's rebuilt as `FIXED` at whatever it resolved to, see `InvoiceDraftStore.loadFromInvoice`).
...
- **Platform-specific UI branching** goes through `PlatformService` (`core/services/platform.service.ts`, wraps `Capacitor.getPlatform()`/`isNativePlatform()`), used today to: hide any Stripe checkout-initiating CTA on iOS (Apple 3.1.1's "external subscription, business tool" pattern — `PaywallModalComponent`/`subscribe.page.ts`/`SubscribePage`) and hide Google login on iOS specifically (Apple 4.8 — see roadmap notes). In place of the hidden CTA, those same three iOS branches offer `platformService.openWebSubscriptionPage()`, a button that hands off to `@capacitor/browser`'s `Browser.open()` — the system browser (`SFSafariViewController`), never the app's own WebView — pointed at `https://facturele.net/abonnement`. That link-out stays compliant because it never initiates the payment flow itself; it just leaves the app.
...
- **Store rating prompt (Phase 31)**: `RatingPromptService` (same no-op-on-web guard as above) calls `@capacitor-community/in-app-review`'s native review dialog (`SKStoreReviewController` on iOS, the Play In-App Review API on Android) from `InvoiceShareService.share()`, right after an artisan completes a real invoice send. Entirely client-local — a `localStorage` counter (own key, same precedent as `ManualInvoiceDraftStore`) decides *when this app asks*, never whether the OS actually shows anything; see [roadmap.md](roadmap.md) Phase 31 for why there's deliberately no happy-path pre-screen gating the native call.

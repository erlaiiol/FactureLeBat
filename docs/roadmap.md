...
- Verify at phone width first (docs/front/'s primary real-device context for this app) — first login on a phone is exactly the moment this needs to land well.
...
- [x] The expanded button reveals an inline checklist panel — grown *inside* the folder's own card (docs/front/'s `panelStretch`/"stretch, not swap" principle), not mode rapide's viewport-anchored fixed flyout, since this list lives inside a scrolling grid of many folder cards, not a single full-screen flow — listing every product/prestation/remise of that type, each with a checkbox reflecting whether it currently belongs to this folder
